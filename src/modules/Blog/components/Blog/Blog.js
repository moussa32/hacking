  ...
  const SocialIconsGroup = ({ classNames }) => (
    <div className={`share ${classNames ? classNames : ""}`}>
      <p className="social-icon-group">
        ...
        <SocialIcon icon={TelegramIcon} alt="telegram icon" link={`https://t.me/share/url?url=${blogFullPath}&text=${blog.title}`} />
        <SocialIcon icon={TwitterIcon} alt="twitter icon" link={"https://twitter.com/share?ref_src=twsrc%5Etfw"} />
        <SocialIcon icon={FacebookIcon} alt="facebook icon" link={`https://www.facebook.com/sharer.php?u=${blogFullPath}`} />
      </p>
    </div>
  );
  ...
  return (
    blog !== null && (
      <div className="blog-wrapper">
        <div className="container-fluid home blog-wrapper">
          <div className="container-fluid blog-container">
            ...
            <div className="blog-cover-container my-4 p-4">
              <img src={blog.cover ? blog.cover : ""} className="card-img cover img-fluid" alt="blog cover" loading="lazy" />
            </div>
            ...
            {similarBlogs.length > 0 && (
              <div className="similar-blogs mx-4 mt-4">
                <h3 className="mb-4">مدونات ذات صلة</h3>
                {similarBlogs.map(blog => (
                  <BlogCard key={blog.id} blog={blog} category={categories.filter(category => category.id === blog.category)[0]} />
                ))}
              </div>
            )}
          </div>
