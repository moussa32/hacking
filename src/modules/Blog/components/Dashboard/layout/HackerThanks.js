...
const HackerThanks = ({ thankers }) => {
  return (
    <>
      <div className="jumbotron jumbotron-fluid bg-black rounded py-4">
        <div className="container px-4">
          <h2 className="section-title text-right">{thankers.length === 0 ? ('') : (<GiPrayer className="section-icon mb-1 ml-2" size={"2rem"} />)}لوحة الشكر</h2>
          {thankers.length === 0 ? (<><GiPrayer className="mt-4" size={"4rem"} /><p className="mt-4 lead mb-0">لا يوجد اي شكر بعد</p></>) : (
...
const mapStateToProps = ({ blogs }) => {
  return {
    thankers: blogs.userInfo.thankers,
  };
};

export default connect(mapStateToProps)(HackerThanks);