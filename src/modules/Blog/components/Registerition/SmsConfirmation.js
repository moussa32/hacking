...
const SmsConfirmation = () => {
      ...
      smsCodeRequest
        ...
        .catch(error => {
          ...
          if (error.response.status === 401) {
            ...
            setStatus({ error: "برجاء كتاب الرمز الصحيح" });
  ...
  const onReSend = () => {
    ...
    const reSendCode = async () => {
      const reSendRequest = await axios.post(
        `${dvApiUrl}/auth/hackers/resend-code/`,
          ...
          <div className="jumbotron jumbotron-fluid bg-black rounded pt-4">
            ...
            <div className="container">
              ...
              <small className="d-block">
                لم تتلق الرمز؟{" "}
                <button className="text-lightgreen bg-transparent border-0" onClick={onReSend}>
                  أعد إرسال الرمز
                </button>
              </small>
              <Link className="text-lightgreen mt-3 d-block" to="/mobile-confirmation">
                غير رقم الهاتف
              </Link>
            </div>
          </div>
