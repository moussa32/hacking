          ...
          <div className="col-md-6 signup-form-section bg-black">
            ...
            <form onSubmit={handleSignupProgram}>
              <Input
                ...
                onChange={onTyping}
              />
              {status.usernameError ? (<div className="mt-2 alert alert-danger custom-danger-alert" role="alert">
              ...
              <Input
                ...
                onChange={onTyping}
              />
              {status.emailError ? (<div className="mt-2 alert alert-danger custom-danger-alert" role="alert">
                {status.emailError}
              </div>) : null}
              <div className="form-group">
                <label htmlFor="rcrs-country" className="d-block">الدولة</label>
                <CountryDropdown
                  value={formData.country}
                  id="rcrs-country"
                  className="form-control custom-input country-input"
                  defaultOptionLabel={""}
                  onChange={(val) => setFormData({ ...formData, "country": val })}
                />
              </div>
              {status.countryError ? (<div className="mt-2 alert alert-danger custom-danger-alert" role="alert">
                {status.countryError}
              </div>) : null}
              <div className="form-group">
                <label htmlFor="" className="d-block">رقم الهاتف</label>
                <PhoneInput
                  country={'kw'}
                  name="phone_number"
                  localization={ar}
                  enableSearch={true}
                  inputClass={"w-100"}
                  containerStyle={{ "borderRadius": "5px", "border": "1px #ddd solid" }}
                  onChange={(number) => setFormData({ ...formData, "phoneNumber": number })}
                />
              </div>
              {status.phoneNumberError ? (<div className="mt-2 alert alert-danger custom-danger-alert" role="alert">
              ...
              <Input
                ...
                onChange={onTyping}
              />
              {status.passwordError ? (<div className="mt-2 alert alert-danger custom-danger-alert" role="alert">
              ...
              <Input
                ...
                id="company_name"
                label="اسم الشركة"
                value={formData.program.company_name}
                onChange={(companyName) => setFormData({ ...formData, program: { ...formData.program, 'company_name': companyName.target.value } })}
              />
              {status.companyNameError ? (<div className="mt-2 alert alert-danger custom-danger-alert" role="alert">
              ...
              <Input
                ...
                type="url"
                id="companyUrl"
                label="الرابط الخاص بالشركة"
                value={formData.program.url}
                onChange={(url) => setFormData({ ...formData, program: { ...formData.program, 'url': url.target.value } })} />
              {status.companyURLError ? (<div className="mt-2 alert alert-danger custom-danger-alert" role="alert">
              ...
              {status.accept_rulesError ? (<div className="mt-2 alert alert-danger custom-danger-alert" role="alert">
