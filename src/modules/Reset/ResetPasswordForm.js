...
const ResetPasswordForm = ({ uid, token }) => {
  ...
  const handleResetPasswordForm = e => {
    ...
    if (firstInput.length === 0 || secondInput.length === 0) {
      ...
      axios
        .post(`${dvbaseUrl}/api/v2/auth/users/reset_password_confirm/`, {
          uid: uid,
          token: token,
        ...
        <form className="p-4" onSubmit={handleResetPasswordForm}>
          <div className="form-group">
            <label htmlFor="newPassword">
              <AiFillSafetyCertificate className="text-lightgreen" size={"1.5rem"} /> كلمة المرور الجديده:
            </label>
            <input type="password" placeholder="كلمة المرور" className="form-control custom-input" id="newPassword" aria-describedby="passwordHelp" />
            <small id="passwordHelp" className="form-text text-muted">
              برجاء اختيار كلمة مرور قوية لضمان حمايتك
            </small>
          </div>
          <div className="form-group mt-4">
            <label htmlFor="rePassword">
              <AiFillSafetyCertificate className="text-lightgreen" size={"1.5rem"} /> إعادة كتابة كلمة المرور الجديده
            </label>
            <input type="password" placeholder="كلمة المرور" className="form-control custom-input" id="rePassword" />
          </div>
          <button type="submit" className="btn btn-lightgreen w-50 mx-auto d-block mt-4">
            تغيير
          </button>
        </form>
        {status && (
